	...
	public void deleteCrossing(int crossingId) {
		try {
			...
			String deleteFavoriteCrossingsQuery = "DELETE FROM favorite_crossing WHERE id = ?";
							try (PreparedStatement deleteFavoriteCrossingsStatement = connection.prepareStatement(deleteFavoriteCrossingsQuery)){deleteFavoriteCrossingsStatement.setInt(1, crossingId);
			...
			String deleteRailwayCrossingQuery = "DELETE FROM railway_crossing WHERE id = ?";
							try (PreparedStatement deleteRailwayCrossingStatement = connection.prepareStatement(deleteRailwayCrossingQuery)) {
...
public void addCrossing(RailwayCrossing crossing) {
	...
	public List<RailwayCrossing> getFavoriteCrossings() { 
		...
		try (Connection connection = DatabaseConnection.getConnection()) {
			String query = "SELECT rc.* FROM railway_crossing rc " + "JOIN favorite_crossing fc ON rc.id = fc.id";
			PreparedStatement statement = connection.prepareStatement(query);
			ResultSet resultSet = statement.executeQuery();
			while (resultSet.next()) {
				RailwayCrossing crossing = new RailwayCrossing();
